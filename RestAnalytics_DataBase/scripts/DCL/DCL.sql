...
/*Crea un usuario llamado 'CuevaTomas' con la contraseña especificada en el host 'localhost' 
si el usuario no existe previamente*/
create user if not exists 'CuevaTomas'@'localhost' identified by 'TomasteawitaProyecta2002!';
/*Crea un usuario llamado 'AgustinCastillo' con la contraseña especificada en el host 'localhost'
 si el usuario no existe previamente.*/ 
create user if not exists 'AgustinCastillo'@'localhost' identified by 'Capy78f8d76@e112@@'; 
...
--  Otorga el permiso SELECT en todas las tablas del esquema "RestAnalytics" al usuario 'CuevaTomas' en el host 'localhost'.
GRANT SELECT ON RestAnalytics.* TO 'CuevaTomas'@'localhost'; 

-- Otorga los permisos SELECT, INSERT y UPDATE en todas las tablas del esquema "RestAnalytics" al usuario 'AgustinCastillo' en el host 'localhost'.
GRANT SELECT, INSERT, UPDATE ON RestAnalytics.* TO 'AgustinCastillo'@'localhost'; 

-- Otorga el permiso SELECT en la tabla "sales" del esquema "RestAnalytics" al usuario 'CarlosSaul' en el host 'localhost'.
GRANT SELECT ON RestAnalytics.sales TO 'CarlosSaul'@'localhost'; 
-- Otorga el permiso SELECT en la tabla "customers" del esquema "RestAnalytics" al usuario 'CarlosSaul' en el host 'localhost'.
GRANT SELECT ON RestAnalytics.customers TO 'CarlosSaul'@'localhost'; 
...
-- Otorga el permiso SELECT en todas las tablas del esquema "RestAnalytics" al usuario 'IgnacioZurdo' en el host 'localhost'.
GRANT SELECT ON RestAnalytics.* TO 'IgnacioZurdo'@'localhost'; 

-- Visualizo los resultados finales
